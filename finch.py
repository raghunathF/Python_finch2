...
class Finch():
    ...
    def led_new(self, *args):
        """Control six LEDs (orbs).
            - hex triplet string: led('#00FF0000FF00') or 
            0-255 RGB values: led(0, 255, 139,255,0,134)
        """
        if len(args) == 3:
            lr,lg,lb,rr,rg,rb = [int(x) % 256 for x in args]
        elif (len(args) == 1 and isinstance(args[0], str)):
            color = args[0].strip()
            if len(color) == 13 and color.startswith('#'):
                lr = int(color[1:3], 16)
                lg = int(color[3:5], 16)
                lb = int(color[5:7], 16)
                rr = int(color[7:9], 16)
                rg = int(color[9:11], 16)
                rb = int(color[11:13], 16)
        else:
            return
        self.connection.send(b'O', [lr, lg, lb, rr, rg, rb])
    ...
    def finch_2_buzzer(self, waveform, frequency,volume):

        self.connection.send(b'B',[waveform,(frequency & 0xff00) >> 8, frequency & 0x00ff,volume])
    ...
    def line_follower(self):
        ...
        data = self.connection.receive()
        if data is not None:
            line_follower_value = data[0] 
            return line_follower_value
    ...
    def new_obstacle(self):
        ...
        returns - a tuple(left,right) of two boolean values
        """
        self.connection.send(b'U')
        data = self.connection.receive()
        if data is not None:
            ultrasound = data[1] 
            ultrasound = ultrasound<<8 
            ultrasound = ultrasound | data[0] 
            return ultrasound 
